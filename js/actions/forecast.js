...
import { persistVisibleWeek } from './persist.js';
import { showSuccess } from '../utils/toast.js';

export async function persistForecast(weekKey, { syncCloud = true } = {}) {
...
export async function syncForecastCalendar(reference = new Date()) {
  ...
  if (rollover.changed) {
    ...
    await persistForecast('current', { syncCloud: false });
    await persistForecast('next', { syncCloud: false });
    if (rollover.rotated) {
    ...
    return {
      changed: true,
      rotated: rollover.rotated,
      realigned: Boolean(rollover.realigned),
