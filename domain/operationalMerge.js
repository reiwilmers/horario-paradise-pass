...
/** Fields preserved only while the publisher is actively editing (dirty flag). */
...
/** Dashboard schedule authority — local wins over cloud only during active publisher edits. */
...
export function shouldPushLocalSchedules(local, {
  ...
  const hasNext = scheduleHasAssignments(local?.schedules?.next);
